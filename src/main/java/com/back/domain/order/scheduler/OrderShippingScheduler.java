...
public class OrderShippingScheduler {
    ...
    public void processShippingBefore2PM() {
        ...
        List<OrderStatement> pendingStatements = orderStatementRepository.findByStatusAndCreateDateBefore(
                OrderStatus.PENDING,
                today2PM
        );

        if (pendingStatements.isEmpty()) {
            log.info("오늘 배송할 주문서가 없습니다.");
            return;
        }

        // 일괄 배송 상태로 변경
        for (OrderStatement statement : pendingStatements) {
            statement.markAsShipped();
        }

        log.info("총 {}건의 주문이 당일 배송(SHIPPED) 처리되었습니다.", pendingStatements.size());
    }
}