...
class OrderShippingSchedulerTest {
    ...
    void processShippingBefore2PM_Success() {
        ...
        ReflectionTestUtils.setField(order1, "createDate", before2PM);
        ReflectionTestUtils.setField(order2, "createDate", after2PM);
        ...
        assertThat(savedOrder1.getStatus()).isEqualTo(OrderStatus.SHIPPED);

        // 14시 이후 주문은 여전히 대기 중(PENDING)이어야 함
        assertThat(savedOrder2.getStatus()).isEqualTo(OrderStatus.PENDING);
    }
    ...
    void processShipping_IgnoreAlreadyShipped() {
        ...
        ReflectionTestUtils.setField(order, "createDate", LocalDateTime.now().withHour(10));
        orderRepository.save(order);
    ...
    void expectedShippingDate_Before2PM() {
        ...
        // 필드 강제 주입
        ReflectionTestUtils.setField(order, "createDate", before2PM);

        LocalDate expectedDate = order.getExpectedShippingDate();
    ...
    void expectedShippingDate_After2PM() {
        ...
        // 필드 강제 주입
        ReflectionTestUtils.setField(order, "createDate", after2PM);
